...
async fn open_terminal(t: Instant, host: &str, uuid: &str) {
	...
	async fn read_stdin(t: Instant, tx: WsSender) {
		use termion::raw::IntoRawMode;
		use termion::event::{Event, Key};
		use termion_input_tokio::TermReadAsync;

		let tx = &tx;
		fn send(t: Instant, tx: &futures_channel::mpsc::UnboundedSender<Message>, s: String) {
			tx.unbounded_send(Message::Text(s)).unwrap();
		}

		// Disable line buffering, local echo, etc.
		let mut raw_term: Option<_> = Some(std::io::stdout().into_raw_mode().unwrap());
		//let mraw_term: &mut Option<_> = &mut raw_term;

		tokio::io::stdin()
			.events_stream()
			.take_while(move |event| {
				future::ready(match event {
					Ok(Event::Key(Key::Ctrl('d'))) => {
						// print a newline since echo is disabled
						println!("\r");

						// drop the raw terminal before stopping
						let _ = raw_term.take();
			...
			.for_each(|event_res| async move {
				...
				match event_res {
					Ok(event) => match cmlterm::event_to_code(event) {
						Ok(c) => send(t, &tx, c.to_string()),
	...
	async fn handle_ws_msg(ws_tx: &WsSender, message: Result<Message, WsError>) {
		...
		if let Message::Ping(d) = msg {
			...
			if str_end.ends_with(|c| c == '#' || c == '>' || c == '$') { // IOS+Linux prompts
				/*let last_terminator: Option<usize> = match (str_end.rfind("\n\r"), str_end.rfind("\r\n")) {
					(Some(nr), Some(rn)) => Some(
						std::usize::max(nr, rn)
						//nr.max(rn)
					),
					(Some(nr), None) => Some(nr),
	...
	let (to_server, stdin_rx) = futures_channel::mpsc::unbounded();
	let to_server2 = to_server.clone(); // for pings to be returned
	tokio::spawn(read_stdin(t, to_server));
	...
	let ws_to_stdout = read
		...
		.for_each(|msg| handle_ws_msg(&to_server2, msg));
	...
	to_server2.unbounded_send(Message::Text(ascii::AsciiChar::FF.to_string())).unwrap(); 
